...
for(i in seq(1234, seed, 1000)){
  rest_data <- c(rest_data, readRDS(paste0("rest_data_", i, ".rds")))
}
...
# What if I sampled smaller number of points
id_list <- lapply(rest_data[sample(1:length(rest_data),
                                   1000)],
                  function(x) x$id)
id_list <- unlist(id_list)
length(unique(id_list))
...
t.test(focal$rating, non_focal$rating, alternative = "two.sided",
       paired = F, var.equal = F)
```

```{r}
# Price t-test for focal vs non-focal
t.test(focal$price, non_focal$price, alternative = "two.sided",
       paired = F, var.equal = F)
```

```{r}
# CombinedRate t-test for focal vs non-focal
t.test(focal$CombinedRate, non_focal$CombinedRate, alternative = "two.sided",
       paired = F, var.equal = F)
```

```{r}
# StateRate t-test for focal vs non-focal
t.test(focal$StateRate, non_focal$StateRate, alternative = "two.sided",
       paired = F, var.equal = F)
```

```{r}
# CityRate t-test for focal vs non-focal
t.test(focal$CityRate, non_focal$CityRate, alternative = "two.sided",
       paired = F, var.equal = F)
```

```{r}
# Review Count t-test for focal vs non-focal
t.test(focal$review_count, non_focal$review_count, alternative = "two.sided",
       paired = F, var.equal = F)
```
...
iv_model_2 <- ivreg(rating ~ price  + log(review_count+1951)|
                    log(review_count-min(reg_data$review_count)+1) +
                    StateRate + CountyRate + CityRate + SpecialRate,
                  data = reg_data)
...
iv_model_3 <- ivreg(rating ~ price |
                      StateRate + CountyRate + CityRate + SpecialRate,
                  data = reg_data)
iv_model_4 <- ivreg(rating ~ price  + log(review_count-min(reg_data$review_count)+1)|
                    log(review_count-min(reg_data$review_count)+1) +
                    StateRate + CountyRate + CityRate + SpecialRate,
                  data = reg_data)
...
iv_model_5 <- ivreg(rating ~ price |
                      StateRate + CountyRate + CityRate + SpecialRate,
                  data = reg_data)
iv_model_6 <- ivreg(rating ~ price  + log(review_count-min(reg_data$review_count)+1)|
                    log(review_count-min(reg_data$review_count)+1) +
                    StateRate + CountyRate + CityRate + SpecialRate,
                  data = reg_data)
